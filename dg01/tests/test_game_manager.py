...
class TestGameManager:
    ...
    @pytest.mark.asyncio
    async def test_create_game(self, game_manager, user_id, channel_id):
        session = await game_manager.create_game(user_id, channel_id)
        assert session.user_id == user_id
        assert session.channel_id == channel_id

    @pytest.mark.asyncio
    async def test_end_game(self, game_manager, user_id, channel_id):
        session = await game_manager.create_game(user_id, channel_id)
        game_manager.sessions[(user_id, channel_id)] = session
        await game_manager.end_game(user_id, channel_id)

    @pytest.mark.asyncio
    async def test_handle_game_started(self, game_manager, user_id, channel_id):
    ...
    @pytest.mark.asyncio
    async def test_handle_game_error(self, game_manager, user_id, channel_id):
